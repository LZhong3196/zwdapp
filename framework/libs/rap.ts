/**
 *  RAP utils powered by @author lion
 */
...
const cache: HashMap<Promise<RAP>> = {};
...
export async function getModel(host: string, projectId: number, mode: number) {
    ...
    const ret = cache[url] = _getModel(url, host, projectId, mode);
    return await ret;
}
...
async function _getModel(url: string, host: string, projectId: number, mode: number) {
    ...
    .catch(async (reason) => {
        try {
            fromCache = true;
            return JSON.parse(await RFS.readFile(CACHE_DIR + encodeURIComponent(url)));
        } catch (e) {
            fromCache = false;
            throw reason;
        }
    ...
    let whiteList = await requestPromise;

    if (!fromCache) {
        RFS.writeFile(CACHE_DIR + encodeURIComponent(url), JSON.stringify(whiteList), "utf8");
    } else {
            ...
            for (p in oa) {
                if (oa.hasOwnProperty(p)) {
                    if (!ob.hasOwnProperty(p)) {
                        ...
                        let length = la.length > lb.length ? la.length : lb.length;
